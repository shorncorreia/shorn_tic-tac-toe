$(document).ready(function () {
	...
	var move = 1;
	var play = true;
	$('body').append('<button onclick="location.reload();" id="reset">Reset</button>');
					$('#reset').css('color','#e85a4f');

	$("#board tr td").click(function () {
		if ($(this).text() == "" && play) {
			if ((move % 2) == 1) {
				$(this).append("X");
				$(this).css('color', "#28df99");
			...
			if (checkForWinner() != -1 && checkForWinner() != "") {
				if (checkForWinner() == "X") {
					$('body').append('<div class="winner"><span>Congratulations the Winner is</span>X</div><button onclick="location.reload();" id="reload">Play Again</button>');
					$('.winner').css('background-color', '#28df99');
					$('#reload').css('color','#28df99');
				} else {
					$('body').append('<div class="winner"><span>Congratulations the Winner is</span>O</div><button onclick="location.reload();" id="reload">Play Again</button>');
					$('.winner').css('background-color', '#fddb3a');
					$('#reload').css('color','#fddb3a');
				}
				play = false;
			}
		}
